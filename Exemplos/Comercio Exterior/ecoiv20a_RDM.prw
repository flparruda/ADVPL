...
If lRetIV200
   cPathCont := ""//ALLTRIM(EasyGParam("MV_PATH_CO"))
/*
...
If MsFile( cPathCont+"DI100" ) //FILE(cPathCont+("DI100.DBF"))
   MsErase(cPathCont+"DI100",,"TOPCONN") //FErase(cPathCont+("DI100.DBF"))
EndIf				  

If MsFile( cPathCont+"IV100" )//FILE(cPathCont+("IV100.DBF"))
   MsErase(cPathCont+"IV100",,"TOPCONN")//FErase(cPathCont+("IV100.DBF"))
EndIf				  

If MsFile( cPathCont+"IVH100" )//FILE(cPathCont+("IVH100.DBF"))
   MsErase(cPathCont+"IVH100",,"TOPCONN")//FErase(cPathCont+("IVH100.DBF"))
EndIf				  

If MsFile( cPathCont+"APD100" )//FILE(cPathCont+("APD100.DBF"))
   MsErase(cPathCont+"APD100",,"TOPCONN")//FErase(cPathCont+("APD100.DBF"))
EndIf				  
                                   
If lExisteECF
   If MsFile( cPathCont+"ANT100" )//FILE(cPathCont+("ANT100.DBF"))
      MsErase(cPathCont+"ANT100",,"TOPCONN")//FErase(cPathCont+("ANT100.DBF"))
   EndIf				  
Endif   
...
Do While .T.
   ...
   //FErase(cPathCont+"DI100"+TEOrdBagExt())
   //cPathCont := E_CriaTrab(, aDBF_DI, "Di_Ctb")
   ECOLoadWrk( cPathCont+"DI100" , "DI_Ctb" , aDBF_DI )
   IF ! USED()
      IVMsgAlert(STR0013+cPathCont+"DI100",STR0014) //"Não foi possivel abrir o arquivo "###"Atenção"
      lRet20B:=.F.
   ENDIF
   //IndRegua("DI_Ctb",cPathCont+"DI100"+TEOrdBagExt(),"DIHAWB")
   ECOIVIndRg( {{"DI_Ctb" , cPathCont+"DI100" , {"DIHAWB"} }}   )

   //FErase(cPathCont+"IVH100"+TEOrdBagExt())
   //cPathCont := E_CriaTrab(, aDBF_IVH, "IVH_Ctb")
   ECOLoadWrk( cPathCont+"IVH100" , "IVH_Ctb" , aDBF_IVH )
   IF ! USED()
      DI_Ctb->(DbCloseArea())
   ...
   //FErase(cPathCont+"IV100"+TEOrdBagExt())
   //FErase(cPathCont+"IV102"+TEOrdBagExt())
   //cPathCont := E_CriaTrab(, aDBF_IV, "IV_Ctb")
   ECOLoadWrk( cPathCont+"IV100" , "IV_Ctb" , aDBF_IV )
   IF ! USED()
      DI_Ctb->(DbCloseArea())
      IVH_Ctb->(DbCloseArea())
      ...
      lRet20B:=.F.
   ENDIF
   //IndRegua("IV_Ctb",cPathCont+("IV100"+TEOrdBagExt()),"IVINVOICE+IVIDENTCT")
   //IndRegua("IV_Ctb",cPathCont+("IV102"+TEOrdBagExt()),"IVINVOICE+IVFORN+IVIDENTCT")  
                                                 
   //SET INDEX TO (cPathCont+("IV100"+TEOrdBagExt())),(cPathCont+("IV102"+TEOrdBagExt()))         
   ECOIVIndRg( {{"IV_Ctb" , cPathCont+"IV100" , { "IVINVOICE+IVIDENTCT" , "IVINVOICE+IVFORN+IVIDENTCT" , "IVFORN+IVIDENTCT" } }}   )

   //cPathCont := E_CriaTrab(, aDBF_APD, "APD_Ctb")
   ECOLoadWrk( cPathCont+"APD100" , "APD_Ctb" , aDBF_APD )
   IF ! USED()
      DI_Ctb->(DbCloseArea())
      IVH_Ctb->(DbCloseArea())
      IV_Ctb->(DbCloseArea())
      ...
      lRet20B:=.F.
   ENDIF
   //IndRegua("APD_Ctb",cPathCont+("APD100"+TEOrdBagExt()),"APDHAWB+APDTIPOREG+APDINVOICE")
   ECOIVIndRg( {{"APD_Ctb" , cPathCont+"APD100" , { "APDHAWB+APDTIPOREG+APDINVOICE" } }}   )
   
   If lExisteECF

      //cPathCont := E_CriaTrab(, aANT_ANT, "ANT_Ctb")
      ECOLoadWrk( cPathCont+"ANT100" , "ANT_Ctb" , aANT_ANT )
      IF ! USED()
         DI_Ctb->(DbCloseArea())
         IVH_Ctb->(DbCloseArea())
         IV_Ctb->(DbCloseArea())
      ...
      //IndRegua("ANT_Ctb",cPathCont+("ANT100"+TEOrdBagExt()),"ANTFORN+ANTINVOICE+ANTIDENTCT")
      ECOIVIndRg( {{"ANT_Ctb" , cPathCont+"ANT100" , { "ANTFORN+ANTINVOICE+ANTIDENTCT" } }}   )
   Endif
...
SX2->(DbSeek('EC2'))
cArqSQL := AllTrim(RetSQLName("EC2"))
...
// EC3
SX2->(DbSeek('EC3'))
cArqSQL := AllTrim(RetSQLName("EC3"))
...
// EC4
SX2->(DbSeek('EC4'))
cArqSQL := AllTrim(RetSQLName("EC4"))
...
// EC5
SX2->(DbSeek('EC5'))
cArqSQL := AllTrim(RetSQLName("EC5"))
...
// EC7
SX2->(DbSeek('EC7'))
cArqSQL := AllTrim(RetSQLName("EC7"))
...
// EC9
SX2->(DbSeek('EC9'))
cArqSQL := AllTrim(RetSQLName("EC9"))
...
If !Empty(cMv)
   If EasyGParam(cMv,,cDefault) == "1" //Encerramento
      cRet   := "W6_DT_ENCE"
   ElseIf EasyGParam(cMv,,cDefault) == "2" //Embarque
      cRet   := "W6_DT_EMB"
   ElseIf EasyGParam(cMv,,cDefault) == "3" //Registro DI
      cRet   := "W6_DTREG_D"
   ElseIf EasyGParam(cMv,,cDefault) == "4" //Emissão NF
      cRet   := "W6_DT_NF"
   ElseIf EasyGParam(cMv,,cDefault) == "5" //Data de Entrega
      cRet   := "W6_DT_ENTR"
   EndIf
EndIf
...
/*
Funcao      : ECOLoadWrk
Parâmetros  : aInd -> { cFileWork  = Nome do arquivo físico da tabela
                        cAlias     = Alias da Tabela
                        aStruct    = Array com a estrutura da tabela a ser criada }
Retorno     : lRet
Objetivos   : Criar tabela temporária permanente no banco de dados ou usar a tabela já criada
Autor       : Nilson César
Data/Hora   : 08/08/2018
*/
Static Function ECOLoadWrk( cFileWork , cAlias , aStruct )

If MsFile(cFileWork)
   DbUseArea( .T. , "TOPCONN" , cFileWork , cAlias , .F. , )     
Else
   MsCreate( cFileWork,aStruct,"TOPCONN" )
   DbUseArea( .T. , "TOPCONN" , cFileWork , cAlias , .F. , )
EndIf
...
*/
STATIC FUNCTION ECOIVIndRg( aInd )
