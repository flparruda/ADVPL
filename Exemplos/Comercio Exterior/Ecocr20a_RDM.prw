...
cPath := ""/*AllTrim(EasyGParam("MV_PATH_CO"))*/; cMesProc := AllTrim(EasyGParam("MV_MESPROC"))
...
If lConecTop
  IF EC9->(EasyRecCount())/1024 < 1
     nCount := 1
  Else
     nCount := Round(EC9->(EasyRecCount())/1024,0)
  EndIf
  ProcRegua(nCount)
  SX2->(DBSEEK("EC9"))
  nCount:=EC9->(EasyRecCount())

  nCont := 0
  // APAGA A CADA 1024 PARA NÃO ENCHER O LOG DO BANCO!
  While nCont <= nCount
       IncProc(STR0065)
       cQuery := "DELETE FROM "+RetSQLName("EC9")+" "
...
If lConecTop
   IF EC7->(EasyRecCount())/1024 < 1
      nCount := 1
   Else
      nCount := Round(EC7->(EasyRecCount())/1024,0)
   EndIf
   ProcRegua(nCount)
   SX2->(DBSEEK("EC7"))
   nCount:=EC7->(EasyRecCount())

   nCont := 0
   // APAGA A CADA 1024 PARA NÃO ENCHER O LOG DO BANCO!
   While nCont <= nCount
        IncProc(STR0066)
        cQuery := "DELETE FROM "+RetSQLName("EC7")+" "
    	cQuery := cQuery + " WHERE R_E_C_N_O_ between "+Str(nCont)+" AND "+Str(nCont+1024)
