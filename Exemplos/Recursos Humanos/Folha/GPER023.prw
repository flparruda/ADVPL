...
Static Function fGeraVincu()
	...
	cQuery := "SELECT a.RD_MAT AS MAT,b.RA_XNOMECO AS NOME,c.Q3_DESCSUM AS CARGO, b.RA_SEXO AS SEXO, a.RD_PD AS VERBA,a.RD_VALOR AS VALOR,a.RD_DATARQ AS DATAARQ," + chr(13)
	...
	While !TEMP->(EOF())
		...
		AADD(aItem,Alltrim(TEMP->NOME))															//04-Nome do Servidor
		AADD(aItem,Alltrim(TEMP->SEXO))															//05-Sexo
		AADD(aItem,TEMP->MAT)																	//06-Matrícula
		...
		AADD(aItem,Gf(TEMP->FUNCAO))															//12-Função
...
Static Function fGeraRubri()
	...
	cQuery += " WHERE b.RV_TIPOCOD IN ('1','2') AND b.RV_TRANSPA = '1' AND a.RD_PERIODO = '" + MV_PAR02 + "'" + chr(13)	
	cQuery += " AND a.D_E_L_E_T_ <> '*' AND b.D_E_L_E_T_ <> '*' AND c.D_E_L_E_T_ <> '*' " + chr(13)
